            ...
            function (rv) {
                if (success) {
                    if (rv.length) {
                        ...
                        for (var i = 0, _len = rv.length; i < _len; i++) {
                            var xmlrpc = rv[i].xmlrpc;
                            var apiUrl = params.apiUrl;

                            // wordpress.com site shows an http link, while pinging their API
                            // returns an https link, so this strips both, so we're just matching
                            // the domain and URI
                            xmlrpc = xmlrpc.substring(xmlrpc.indexOf('://') + 3);
        ...
        XMLRPC_LIB.doCommand(
            ...
            function (rv) {
                ...
                for (var i = 0, _len = rv.events.length; i < _len; i++) {
                    ...
                    post.content = utf8_decode(post.content
                        .replace(/<a name=.cutid1.><\/a>/, '<!--more-->')
                        .replace(/<a name=.cutid1-end.><\/a>/, '<!--endmore-->'));

                    post.title = utf8_decode(event.subject);
...
var genericAtomAPI = function () {
    ...
    this.publish = function (params, success, failure) {
        ...
        body += '<entry xmlns="http://www.w3.org/2005/Atom" xmlns:app="http://www.w3.org/2007/app#">';
                    ...
                    if (req.status < 300) {
                        ...
                        switch (returnValues["Error"]) {
                            ...
                            case 'CaptchaRequired':
                                ...
                                $('#scribefire-loading-notice').html(container).dialog();
