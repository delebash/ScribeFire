var SCRIBEFIRE = {
    currentAPI : null,
    version: '4.3.5',
    apiUrl: 'http://www.scribefire.com/api/',
    ...
    load : function () {
        ...
        if (($("#list-blogs option").length == 0)) {
            if (!SCRIBEFIRE.prefs.getBoolPref("firstrun")) {
                $('#scribefire-loading-notice').html(
                    $("<div/>")
                    .append(
                        $("<h2/>")
                        .addClass("i18n")
                ...
                ).dialog({width: 600, height:400, modal: true, resizable: false});

                SCRIBEFIRE.prefs.setBoolPref("firstrun", true);
            }
        }

        SCRIBEFIRE.observe("rtl");
    },

    observe: function(prefName) {
        switch (prefName) {
            case "rtl":
    ...
    newWindow : function() {
        return SCRIBEFIRE.prefs.getBoolPref('newWindow');
    },
    viewBlog : function () {
    ...
    populateBlogsList : function () {
        ...
        var oldSelectedBlog = SCRIBEFIRE.prefs.getCharPref("selectedBlog");
        ...
        for (var i in blogs) {
            ...
            if (blogs[i].url) {
                ...
                var label = blogs[i].name + " (" + cleanUrl(blogs[i].url) + ")";

                for (var j in blogs) {
                    if (j != i && blogs[j].url == blogs[i].url && blogs[i].username) {
                        label = blogs[i].name + " ("+blogs[i].username + " @ " + cleanUrl(blogs[i].url)+")";
                        break;
                    }
                }
            }
            ...
            if (i == oldSelectedBlog || newSelectedBlog == null) {
                newSelectedBlog = i;
            }
        ...
        if (count > 0) {
            $("#list-blogs").val(newSelectedBlog);
            $("#list-blogs").show();
        } else {
            $("#list-blogs").hide();
            $(".blog-meta").hide();
        ...
        if (newSelectedBlog) {
            SCRIBEFIRE.prefs.setCharPref('selectedBlog', newSelectedBlog);
        }
    ...
    getBlogMetaData : function (url, callbackSuccess, callbackFailure, secondTry) {
        ...
        if (parsed.host.search(/\.tumblr\.com$/i) != -1) {
            ...
            metaData.type = 'atom';
            metaData.apiUrl = 'http://www.dreamwidth.org/interface/atom';
        } else if (parsed.host.search(/\.livejournal\.com$/i) != -1) {
            metaData.type = 'livejournal';
            metaData.apiUrl = 'http://www.livejournal.com/interface/xmlrpc';
        }
                    ...
                    if (req.status < 300) {
                        ...
                        if (linkTags) {
                            for (var i = 0, _len = linkTags.length; i < _len; i++) {
                                ...
                                if (link_rel && link_rel.match(/^service\./i) && link.attr('type') == 'application/atom+xml') {
                                    atomAPIs[link_rel.toLowerCase()] = link.attr("href");

                                    metaData.type = "atom";
                                } else if (!metaData.type) {
                                    if (link_rel == "pingback" && link.attr("href")) {
                            ...
                            if ("service.post" in atomAPIs) {
                                ...
                                if (metaData.type == "blogger") {
                                    metaData.apiUrl = metaData.apiUrl.replace('http://', 'https://');
                                }
                                callbackSuccess(metaData);
                                return;
                            }
    ...
    publish : function (callbackSuccess, callbackFailure) {
        ...
        */
        ...
        function success(rv) {
            ...
            params.action = 'new_post';
            params.success = true;
            params.isPage = ('pages' === entryType);
            SCRIBEFIRE.track(params);
    ...
    track: function(params) {
        var jsonParams = {
            ...
            browser: browser,
            version: SCRIBEFIRE.version
        }

        if ('new_post' === params.action) {
            var blog = SCRIBEFIRE.getBlog();
            jsonParams.blogType = blog.type;
            jsonParams.blogApiUrl = blog.apiUrl;
            jsonParams.blogUrl = blog.url;

            jsonParams.isPage = params.isPage;
        } else {
            jsonParams.blogType = params.type;
            jsonParams.blogApiUrl = params.apiUrl;
            jsonParams.blogUrl = params.blogUrl;
        }

        $.post(SCRIBEFIRE.apiUrl, {
            p: btoa(JSON.stringify(jsonParams))
        });
    },
    ...
    getBlogs : function (params, callbackSuccess, callbackFailure) {
        getBlogAPI(params.type, params.apiUrl).init(params).getBlogs(
            ...
            function success(rv) {
                ...
                params.action = 'new_blog';
                params.blogUrl = blog.url;
                params.success = true;
                SCRIBEFIRE.track(params);
    ...
    error : function (msg, errorCode) {
        ...
        if ($("#dialog-blog-add").is(":visible")) {
            ...
            */
            ...
            $('#scribefire-loading-notice').html(container).dialog({width: 600, height:400, modal: true, resizable: false});
        }
    },
...
var SCRIBEFIRE_ACCOUNT_WIZARD = {
    ...
    urlCheck: function() {
        var button = $('#button-blog-urlcheck');
...
//											$("#text-oauth-token").val(token);
    ...
    loginCheck: function() {
        var button = $("#button-blog-logincheck");
    ...
    add: function() {
        ...
        $("#dialog-blog-add").dialog({width: 600, height: 400, modal: true, resizable: false});
        ...
        $("#dialog-blog-add").find(".step-1 .subbar").click();
